...
def calc_one_LTE_one_species(
    ...
    J_UP = np.array([t.replace('--',' ').split()[0] for t in out_table_LTE['Line']]).astype(float)
    J_LOW = np.array([t.replace('--',' ').split()[-1] for t in out_table_LTE['Line']]).astype(float)
    out_table_LTE.remove_columns(['Line', 'I_nu_tau0'])
    out_table_LTE.rename_column('T_ant', 'T_RJ')
    out_table_LTE.rename_column('Freq', 'FREQ')
    out_table_LTE.add_column(J_UP, name='J_UP', index=0)
    out_table_LTE.add_column(J_LOW, name='J_LOW', index=1)
    out_table_LTE.add_column(2.99792458e5/(out_table_LTE['FREQ']/1e9), name='WAVE', index=4)
...
def calc_one_RADEX_one_species(
    ...
    if not os.path.isfile(radex_data_file):
        shutil.copy2(os.path.join(radex_data_path, radex_data_file), 
                     os.path.join(radex_calc_path, radex_data_file))
    if radex_calc_path != '':
    ...
    if not os.path.isfile(os.path.join(working_dir, script_name + '.out')):
        command_str = 'cd "%s"; %s < %s > %s'%(working_dir, radex_bin_path, 
...
def run_for_all_input_sets():
    ...
    for idx_X_CICO, X_CICO in enumerate(list_of_X_CICO):
        ...
        for idx_d_V, d_V in enumerate(list_of_d_V):
            ...
            out_table_name = 'out_table_RADEX_R_CI10CO21_sim_XCICO_%.1f_dv_%.1fkms'%(X_CICO, d_V)
