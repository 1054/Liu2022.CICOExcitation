...
radex_bin_path = os.path.join(os.path.dirname(os.getcwd()), 'bin', 'radex_LVG_linux') # disky
radex_data_path = os.path.join(os.path.dirname(os.getcwd()), 'data')
radex_calc_path = os.path.join(os.path.dirname(os.getcwd()), 'tmp', 'calc_radex_disky') # disky
LTE_calc_path = os.path.join(os.path.dirname(os.getcwd()), 'tmp', 'calc_LTE')
#CPU_cores = 2
CPU_cores = 160 # disky
#import socket
#import multiprocessing
#if socket.gethostname() in ['disky']:
#    CPU_cores = int(np.ceil(multiprocessing.cpu_count() * 0.8))
#print('CPU_cores: {}'.format(CPU_cores))
...
list_of_n_H2 = [1.00e2, 3.16e2, 1.00e3, 3.16e3, 1.00e4, 3.16e4, 1.00e5] # disky
...
debug_test = False
if debug_test:
    list_of_n_H2 = [3.16e2] # for debuging
    list_of_d_V = [5.] # for debuging

column_formats = {'n_H2': '{:.1e}'}  # for testing 20220310a
...
# dzliu moldata object for LTE(/nonLTE) calculation
...
# Define function to compute LTE solutions for one input set (T_kin, N_CO/d_V, n_H2, X_CICO)
# and store into a cache database under the LTE_calc_path/ directory.
...
def calc_one_LTE_one_species(
    ...
    out_table_LTE['J_UP'] = np.array([t.replace('--',' ').split()[0]                                       for t in out_table_LTE['Line']]).astype(float)
    out_table_LTE['J_LOW'] = np.array([t.replace('--',' ').split()[-1]                                        for t in out_table_LTE['Line']]).astype(float)
...
# Define function to compute non-LTE solutions for one input set (T_kin, N_CO/line_width, n_H2, X_CICO)
# and store into a cache database under the radex_calc_path/ directory.
...
def calc_one_RADEX_one_species(
    ...
    if not os.path.isfile(calc_inp_file) or True: # disky
...
def run_for_one_input_set(
    ...
    res_dict['Flux_Kkms_CO32_NonLTE'] = out_table_CO_NonLTE['Flux_Kkms'][2]
    res_dict['Flux_Kkms_CO32_LTE'] = out_table_CO_LTE['Flux_Kkms'][2]
    res_dict['Flux_Kkms_CO43_NonLTE'] = out_table_CO_NonLTE['Flux_Kkms'][3]
    res_dict['Flux_Kkms_CO43_LTE'] = out_table_CO_LTE['Flux_Kkms'][3]
    ...
    res_dict['tau_0_CO32_NonLTE'] = out_table_CO_NonLTE['tau_0'][2]
    res_dict['tau_0_CO32_LTE'] = out_table_CO_LTE['tau_0'][2]
    res_dict['tau_0_CO43_NonLTE'] = out_table_CO_NonLTE['tau_0'][3]
    res_dict['tau_0_CO43_LTE'] = out_table_CO_LTE['tau_0'][3]
    ...
    res_dict['Pop_u_CO32_NonLTE'] = out_table_CO_NonLTE['Pop_u'][2]
    res_dict['Pop_u_CO32_LTE'] = out_table_CO_LTE['Pop_u'][2]
    res_dict['Pop_u_CO43_NonLTE'] = out_table_CO_NonLTE['Pop_u'][3]
    res_dict['Pop_u_CO43_LTE'] = out_table_CO_LTE['Pop_u'][3]
    ...
    res_dict['Pop_l_CO32_NonLTE'] = out_table_CO_NonLTE['Pop_l'][2]
    res_dict['Pop_l_CO32_LTE'] = out_table_CO_LTE['Pop_l'][2]
    res_dict['Pop_l_CO43_NonLTE'] = out_table_CO_NonLTE['Pop_l'][3]
    res_dict['Pop_l_CO43_LTE'] = out_table_CO_LTE['Pop_l'][3]
    ...
    res_dict['T_ex_CO32_NonLTE'] = out_table_CO_NonLTE['T_ex'][2]
    res_dict['T_ex_CO32_LTE'] = out_table_CO_LTE['T_ex'][2]
    res_dict['T_ex_CO43_NonLTE'] = out_table_CO_NonLTE['T_ex'][3]
    res_dict['T_ex_CO43_LTE'] = out_table_CO_LTE['T_ex'][3]
...
def write_table_to_disk(out_table, out_table_name):
    ...
    if not os.path.isdir(radex_calc_path):
        os.makedirs(radex_calc_path, exist_ok=True)
...
# Define lookup key format function for data table names
...
def run_for_all_input_sets():
    ...
    for idx_X_CICO, X_CICO in enumerate(list_of_X_CICO):
        ...
        for idx_d_V, d_V in enumerate(list_of_d_V):
            ...
            # debug
            if debug_test:
                raise NotImplementedError() # break here so that we do not write to disk
